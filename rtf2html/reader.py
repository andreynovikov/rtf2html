...
class Rtf15Reader:
    ...
    def __init__(self, source, errors='strict', clean_paragraphs=True):
        ...
        self.document = document.Container()
    ...
    def go(self):
        ...
        if self.source.read(5) != br"{\rtf":
            raise RuntimeError("Doesn't look like an RTF file")
    ...
    def parse(self):
        while True:
            ...
            if nextbyte in b'\r\n':
                continue
            if nextbyte == b'{':
                sub_group = Group(self, self.group, self.charsetTable)
                self.stack.append(sub_group)
                sub_group.skip = self.group.skip
                self.group.flush_chars()
                self.group = sub_group
            elif nextbyte == b'}':
                sub_group = self.stack.pop()
                self.group = self.stack[-1]
                if self.group.specialMeaning == 'FONT_TABLE':
                    sub_group.ignore()  # else font names may appear as text
                sub_group.finalize()

                if sub_group.specialMeaning == 'HTML_TAG':
                    sub_group.tag.content = b"".join(sub_group.charBuffer).decode(sub_group.charset, sub_group.reader.errors)
                    sub_group.skip = False

                if sub_group.specialMeaning == 'FONT_TABLE':
                    self.charsetTable = sub_group.charsetTable
                self.group.content.append(sub_group)
                ...
                control, digits = self.get_control()
    ...
    def build(self):
        ...
        for bit in self.group.flatten():
            type_name = type(bit).__name__
            getattr(ctx, "handle_%s" % type_name)(bit)
...
class DocBuilder(object):
    ...
    def flushRun(self):
        if self.block is None:
            self.block = document.Container()

        if self.isImage:
            self.block.content.append(
                document.Image(self.propStack[-1].copy(), [b"".join(self.run)]))
            self.isImage = False
        elif self.isHtml:
            self.block.content.append(
                document.Html(self.propStack[-1].copy(), [u"".join(self.run)]))
            self.isHtml = False
        else:
            self.block.content.append(
    ...
    def handle_HtmlTag(self, marker):
        self.flushRun()
        self.isHtml = True
        if marker.val:
            self.propStack[-1][marker.name] = marker.val
            self.run.append(marker.content)  # htmltag marker has additional content field
...
class Group:
    def __init__(self, reader, parent=None, charset_table=None):
        self.reader = reader
        self.parent = parent
        ...
        self.charsetTable = charset_table
    ...
    def handle(self, control, digits):
        if self.htmlrtf and control != b"htmlrtf":
            return

        if self.charBuffer and control != b"ansi_escape" and self.specialMeaning != 'HTML_TAG':  # end of a text range
            self.flush_chars()
    ...
    def _setFontCharset(self, charset=None):
        ...
        if self.parent.charsetTable is not None:
            self.parent.charsetTable[self.font_num] = charset
        else:
            self.charsetTable[self.font_num] = charset
    ...
    def handle_f(self, font_num):
        if 'FONT_TABLE' in (self.parent.specialMeaning, self.specialMeaning):
            ...
            try:
                self.charset = self.charsetTable[int(font_num)]
            except KeyError:
    ...
    def handle_fcharset(self, charset_num):
        if 'FONT_TABLE' in (self.parent.specialMeaning, self.specialMeaning):
            ...
            charset = _CODEPAGES.get(int(charset_num))

            if charset is None:
                raise ValueError("Unsupported charset %s" % charset_num)
    ...
    def handle_control_symbol(self, symbol):
        ...
        assert len(symbol) == 1
        if symbol in b'\\{}':
            if self.specialMeaning == 'HTML_TAG':
                self.charBuffer.append(symbol)
            else:
    ...
    def handle_strike(self, on_off=None):
        val = on_off in (None, "", "1")
    ...
    def handle_htmltag(self, digits):
        p = HtmlTag(digits)
        self.content.append(p)
        self.tag = p
        self.specialMeaning = 'HTML_TAG'
        self.destination = False

    def handle_htmlrtf(self, on_off=None):
        val = on_off in (None, "", "1")
        self.htmlrtf = val
...
class HtmlTag(ReadableMarker):
    def __init__(self, digits):
        ReadableMarker.__init__(self, "htmltag", digits)
        self.content = None
